...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, q, k, v, mask, incidence_matrix=None):
...
def scaled_dot_product_attention(q, k, v, mask=None, incidence_matrix=None):
...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask, incidence_matrix=None):
...
class Encoder(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask, incidence_matrix=None):
        ...
        for i in range(self.num_layers):
            x = self.enc_layers[i](x, training, mask, incidence_matrix)
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training,
        ...
        attn2, attn_weights_block2 = self.mha2(enc_output,
            enc_output, attn1, padding_mask)  # (batch_size, target_seq_len, d_model)
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, look_ahead_mask, padding_mask, voc_size):

        seq_len = tf.shape(x)[1]
        attention_weights = {}

        x *= tf.math.sqrt(tf.cast(self.d_model, tf.float32))  # (batch_size, target_seq_len, d_model)
